  ...
  return (
    <article
      ...
      className={`
        ...
        hover:-translate-y-1 hover:shadow-xl focus-within:ring-2 focus-within:ring-accent-purple
        ${plan.popular
    ...
    >
      ...
      <div className="mb-4">
        ...
        <p className="text-xs text-text-muted mt-0.5">{plan.tagline}</p>
      </div>
      ...
      >
        {price === null ? (
          ...
          <div className="flex items-end gap-1">
            ...
            <span className="text-sm text-text-muted mb-1.5">/ mo</span>
        ...
        {plan.slug === "team" && (
          <p className="text-xs text-text-muted mt-1">Per seat · custom quote</p>
        )}
      </div>

      {/* Credit label chip */}
      <div className="mb-4 inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-surface-light/50 border border-border text-[11px] font-semibold text-text-secondary">
        ...
        {plan.features.map((feat) => (
          <li
            ...
            className={`flex items-start gap-2.5 text-xs leading-relaxed ${
              feat.included ? "text-text-primary/80" : "text-text-muted/40"
            }`}
          >
            <span
              aria-hidden="true"
