  ...
  if (typeof value === "boolean") {
    return value ? (
      ...
      <span aria-label="not included">
        <svg className="w-4 h-4 mx-auto text-text-muted/40" viewBox="0 0 16 16" fill="none">
        ...
        <tbody>
          {ROWS.map((row, idx) => {
            if (isCategory(row)) {
              return (
                <tr key={`cat-${idx}`}>
                  <td
                    colSpan={5}
                    className="px-5 pt-5 pb-2 text-[10px] font-bold uppercase tracking-[0.12em] text-text-muted"
                  >
                    {row.category}
                  </td>
                </tr>
            ...
            return (
              <tr
                key={feat.feature}
                className="border-t border-border/50 hover:bg-surface-light/30 transition-colors"
              >
